...
// Main function to get media links with priority: Supabase → Videasy → MoviesAPI fallback
export async function getMediaLinks(
        ...
        if (entryType === "movie" || type === "movie") {
          ...
          if (seasonData) {
            ...
            if (seasonData.watch_links && seasonData.watch_links.length > 0) {
              result.seasonEpisodeLinks = seasonData.watch_links.map((link) => extractIframeSrc(link) || link);
              result.found = true;
              result.source = "supabase";
  ...
  // Fallback #1 (default): Videasy
  result.watchUrl = getVideasyFallbackUrl(tmdbId, type, season, episode);
  result.source = "videasy";
  result.found = false;

  // Fallback #2 (optional switch in UI): MoviesAPI
  const moviesApiFallback = getMoviesApiUrl(tmdbId, type, season, episode);

  console.log(`[MediaLinks] Using Videasy fallback for TMDB ${tmdbId}:`, {
    videasy: result.watchUrl,
    moviesApiFallback,
  });
